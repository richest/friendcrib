...
const TableContent = ({ data, saveThought, dispatch , history }) => {
    return <tr>
        ...
        <td align="right">
            <span className="icon cursor-pointer" onClick={() => history.push(`/new_addition/${data.id}/edit/4`)}>
    ...
    return (
        <div className="content__inner">
            <div className="page-heading">
                <h2 className="h3">Restaurants</h2>
            </div>
            <ContentBoxComponent table={{
                ...
                list: props.restaurdantTableProps,
                class: "table custom-table four-col-table"
            }} loading={props.loadingData}
            checkListLengthProps={props.restaurtantLengthProps}
            title="restaurants"
            />
        </div>
    )
