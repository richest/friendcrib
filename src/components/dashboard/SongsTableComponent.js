...
const TableContent = ({ data, dispatch, saveThought , history}) => {
    return <tr>
        ...
        <td align="right">
            <span className="icon cursor-pointer" onClick={() => history.push(`/new_addition/${data.id}/edit/2`)}>
...
const SongsTableComponent = (props) => {
    return (
        <div className="content__inner">
            <div className="page-heading">
                <h2 className="h3">Songs</h2>
            </div>
            <ContentBoxComponent table={{
                ...
                list: props.songTableProps,
                class: "table custom-table four-col-table"
            }}
                loading={props.loadingData} 
                checkListLengthProps={props.songsTableLengthProps}
                title="songs"/>
        </div>
    )
}
